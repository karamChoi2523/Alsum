...
function scene:create( event )
	...
	btn2.alpha = 0.02
	--클릭 유도
	local clickHere = display.newImageRect("content/image/Gong/clickHere.png", 100, 50)
	clickHere.x, clickHere.y = display.contentWidth * 0.388, display.contentHeight * 0.15
	clickHere.alpha = 0
	...
	elseif composer.getVariable("clue8") ==1 then
		--금고 열리는 끼익 소리
		local sound = audio.loadSound("content/audio/open.mp3")
		audio.play(sound)
		btn.alpha = 0
		btn2.alpha = 0
		bankbook.alpha = 1
	...
	local function moving(obj)
		if(safe.alpha ~= 1) then
			clickHere.alpha = 1
			clickHere.y=display.contentHeight * 0.15
			transition.to( clickHere, { time=700, alpha=0.6, y=display.contentHeight * 0.2, onComplete=moving } )
		else
			clickHere.alpha = 0
		end
	end
	...
	local function findSafe(event)
		local sound = audio.loadSound("content/audio/movingBookshelf.mp3")
		audio.play(sound)
		btn.alpha = 0
		clickHere.alpha = 0
		safe.alpha = 1
	...
	local function pwSafe(event)
		...
		pw.alpha = 1
		if(safe.alpha~=1)then
			transition.to( clickHere, { time=700, alpha=0.5, y=display.contentHeight * 0.2, onComplete=moving } )
		else
			clickHere.alpha = 0
		end
	end
