...
local function parse()
	-- 경로를 이용해 파일 가져오기
	local filename = system.pathForFile("content/json/prologue.json")
...
function scene:create( event )
	...
	local function nextScript( ... )
		if(index <= #Data) then
			if(Data[index].type == "Dialogue")then
				speakerImg.alpha = 1
				speakerImg.fill = {
					type = "image",
			...
			composer.setVariable("toMap1", 5)
			composer.setVariable("goToParty", 1)
