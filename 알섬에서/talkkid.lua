...
local function parse()
	-- 경로를 이용해 파일 가져오기
	local filename = system.pathForFile("content/json/talk_with_kid.json")
...
function scene:create( event )
	...
	next.alpha = 0
	--화자 이미지
	local speakerImg = display.newRect(display.contentCenterX, display.contentCenterY,970,1080,"content/font/NanumBarunGothic.ttf")
	--이름
	...
	local function nextScript( ... )
		if(index <= #Data) then
			...
			composer.setVariable("clue1", 1)
			composer.setVariable("clue3", 1)
			composer.setVariable("clue4", 1)
			composer.gotoScene("map3")
			composer.setVariable("gotoMAP3",2)
		end
	end
	nextScript()
